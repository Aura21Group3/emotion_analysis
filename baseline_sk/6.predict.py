...
# X_test_tfidf_vec
bc=BertClient('ws03.mlamp.co')
X_test_tfidf_vec=bc.encode(['你真的太棒了,又拿了金牌!','你就是头猪','今天天气晴天'])

clf.fit(X_train_tfidf_vec,y_train)
print('训练集得分:{}'.format(clf.score(X_train_tfidf_vec,y_train)))
predict=clf.predict(X_test_tfidf_vec)
f=open('./data/predict','w')
for p in predict:
    # f.write(str(p)+'\n')
    print(p)
